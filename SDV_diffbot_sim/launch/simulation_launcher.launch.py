...
robot_model = 'SDV_diffbot'
robot_ns = 'r1' 
pose = ['0.0', '-2.0', '0.01', '0.0'] # X, Y, Z, YAW (Pista: ['0.6', '-0.8', '0.1', '-1.5708'])
robot_base_color = '0.0 1.0 0.0 1.0' #Ign and Rviz color of the robot's main body (rgba)
world_file = 'room.sdf' # empty.sdf, room.sdf

def generate_launch_description():

    this_pkg_path = os.path.join(get_package_share_directory('SDV_diffbot_sim'))
    ...
    bridge = Node(
        ...
        arguments=[             # ign topic -t <topic_name> --info
            ...
            '/'+robot_ns+'/zed_camera/image@sensor_msgs/msg/Image[ignition.msgs.Image',
            '/'+robot_ns+'/zed_camera/points@sensor_msgs/msg/PointCloud2[ignition.msgs.PointCloudPacked',
        ...
        remappings=[   
            ...
            ('/'+robot_ns+'/zed_camera/image', '/'+robot_ns+'/zed_camera/image'),
            ('/'+robot_ns+'/zed_camera/points', '/'+robot_ns+'/zed_camera/points'),
